...
const createBlog = async (req, res, next) => {
    try {
        const { name, role, title, category, intro, content, userProfileUrl } = req.body;
        const imageUrl = req.file.path;

        if ( !name || !role || !title || !category || !intro || !content || !userProfileUrl || !imageUrl) {
            const error = createHttpError(400, "All fields are must!");
            return next(error);
        }

        const blog = { name, role, title, category, intro, content, userProfileUrl, imageUrl };
...
module.exports = { createBlog, getBlogById, likeBlogById };